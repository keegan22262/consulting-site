...
function PrideCard({ card }: { card: (typeof PRIDE_VALUES)[number] }) {
  ...
  return (
    <div
      ...
      className="bg-white rounded-xl px-7 py-8 transition-all duration-300 cursor-pointer h-full"
      style={{
        boxShadow: hovered ? "0 12px 32px rgba(2,16,36,0.1)" : "0 4px 20px rgba(2,16,36,0.06)",
        borderTop: "3px solid #5483B3",
        transform: hovered ? "translateY(-4px)" : "translateY(0)",
      }}
    >
      <div className="w-[52px] h-[52px] rounded-full flex items-center justify-center mx-auto" style={{ backgroundColor: "#C1E8FF" }}>
        {card.icon}
      </div>
      <div className="text-center mt-4 text-[28px]" style={{ fontFamily: "var(--font-playfair), 'Playfair Display', serif", fontWeight: 600, color: "#052659" }}>
        {card.letter}
      </div>
      <h3 className="text-center mt-2 text-[18px] font-semibold" style={{ fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif", color: "#021024" }}>
        {card.name}
      </h3>
      <p className="text-center mt-2 text-[13px] leading-[1.6]" style={{ fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif", color: "#6B7280" }}>
        {card.body}
      </p>
    </div>
  );
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-10">
        ...
        <div className="text-center">
          ...
          >
            ...
            architecture that governs how we engage, deliver, and sustain client outcomes.
          </p>
        </div>
        ...
        <div className="hidden md:block">
          <div className="grid grid-cols-3 gap-6">
            {PRIDE_VALUES.slice(0, 3).map((card, i) => (
              <ScrollReveal key={card.letter} direction="up" delay={200 + i * 150} distance={35} duration={800}>
                <PrideCard card={card} />
              </ScrollReveal>
            ))}
          </div>
          <div className="flex justify-center gap-6 mt-6">
            {PRIDE_VALUES.slice(3).map((card, i) => (
              <ScrollReveal key={card.letter} direction="up" delay={650 + i * 150} distance={35} duration={800} className="w-[calc(33.333%-8px)]">
                <PrideCard card={card} />
              </ScrollReveal>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-6 md:hidden">
          {PRIDE_VALUES.map((card, i) => (
            <ScrollReveal key={card.letter} direction="up" delay={100 + i * 100} distance={30} duration={700}>
              <PrideCard card={card} />
            </ScrollReveal>
          ))}
        </div>
