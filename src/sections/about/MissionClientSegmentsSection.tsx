...
const CLIENT_SEGMENTS = [
  ...
  { title: "SMEs", description: "Scaling with structure: operations, compliance, digital enablement, and growth.", borderColor: "#7DA0CA" },
  { title: "Mid-size & Large Organizations", description: "Transformation: operating model, risk, technology, people, and ESG.", borderColor: "#C1E8FF" },
];
...
export default function MissionClientSegmentsSection() {
  ...
  const missionText = useScrollReveal({ direction: "up", delay: 300, distance: 25, duration: 800 });
  const segmentHeading = useScrollReveal({ direction: "up", delay: 400, distance: 20, duration: 700 });
  const imageReveal = useScrollReveal({ direction: "right", delay: 200, distance: 50, duration: 900 });
  const overlayImage = useScrollReveal({ direction: "down", delay: 500, distance: 30, duration: 800 });
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-10">
        <div className="flex flex-col lg:flex-row gap-12 items-center">
          <div className="w-full lg:w-[55%] order-2 lg:order-1">
            ...
            <p
              ref={missionText.ref as React.RefObject<HTMLParagraphElement>}
              className="text-[15px] leading-[1.75]"
              style={{ fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif", color: "#4A4A4A", ...missionText.style }}
            >
            ...
            <div className="mt-8">
              <h3
                ref={segmentHeading.ref as React.RefObject<HTMLHeadingElement>}
                className="text-[20px] font-semibold mb-5"
                style={{ fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif", color: "#021024", ...segmentHeading.style }}
              >
                Client Segments
              </h3>
              ...
              <div className="flex flex-col gap-5">
                {CLIENT_SEGMENTS.map((segment, i) => (
                  <ScrollReveal key={segment.title} direction="left" delay={500 + i * 150} distance={30} duration={700}>
                    <div
                      className="rounded-[10px] px-6 py-5"
                      style={{ backgroundColor: "#F8FBFF", borderLeft: `3px solid ${segment.borderColor}` }}
                    >
                      <h4 className="text-[16px] font-semibold" style={{ fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif", color: "#021024" }}>
                        {segment.title}
                      </h4>
                      <p className="text-[14px] mt-1" style={{ fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif", color: "#6B7280" }}>
                        {segment.description}
                      </p>
                    </div>
                  </ScrollReveal>
